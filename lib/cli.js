...
const pick = (o, ...keys) => Object.assign(
  {},
  ...keys.map(prop => ({ [prop]: o[prop] }))
)
...
const argv = yargs
  ...
  .default('dir', path.join(process.env.HOME, '.config', pack.name))
...
run(Object.assign(pick(argv, ...d1Keys), {
  data: Object.assign(argv.data || {}, omit(argv, 'data', '_', '$0', ...d1Keys))
}))
